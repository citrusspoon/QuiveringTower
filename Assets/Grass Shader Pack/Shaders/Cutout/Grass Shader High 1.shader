Shader "Grass/Cutout/High Quality" {
	Properties {
		...
		void vertCustom (inout appdata_full v, half ITERATION) {
			v.vertex.xyz += v.normal * _Length / _Div * ITERATION;
		}
		...
		/*void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float2 offset = float2 (offsetx, offsety) * scale;
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surfTop Standard alphatest:_Cutoff vertex:vert
		#define ITERATION 15
		void vert (inout appdata_full v, out Input o) {
			UNITY_INITIALIZE_OUTPUT (Input, o);
			half ite = ITERATION;
